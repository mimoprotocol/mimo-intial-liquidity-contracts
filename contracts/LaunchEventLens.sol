...
contract LaunchEventLens {
    ...
    /// @param _rocketMimoFactory Address of the RocketMimoFactory
    constructor(address _rocketMimoFactory) {
        rocketMimoFactory = IRocketMimoFactory(_rocketMimoFactory);
    }
        ...
        returns (LaunchEventData[] memory)
    {
        ...
        uint256 numLaunchEvents = rocketMimoFactory.numLaunchEvents();
        ...
        for (uint256 i = _offset; i < end; i++) {
            address launchEventAddr = rocketMimoFactory.allRMLaunchEvents(i);
    ...
    function getLaunchEventData(ILaunchEvent _launchEvent)
        ...
        returns (LaunchEventData memory)
    {
        (uint256 ethReserve, uint256 tokenReserve) = _launchEvent
            .getReserves();
        ...
        return
            LaunchEventData({
                ...
                ethAllocated: _launchEvent.ethAllocated(),
