...
contract LaunchEvent {
    ...
    IMimoFactory private factory;
    IRocketMimoFactory public rocketMimoFactory;
    ...
    function initialize(
        ...
        rocketMimoFactory = IRocketMimoFactory(msg.sender);
        require(
            _token != rocketMimoFactory.weth() &&
        ...
        WETH = IWETH(rocketMimoFactory.weth());
        factory = IMimoFactory(rocketMimoFactory.factory());

        issuer = _issuer;

        auctionStart = _auctionStart;
        phaseOneDuration = rocketMimoFactory.phaseOneDuration();
        phaseOneNoFeeDuration = rocketMimoFactory.phaseOneNoFeeDuration();
        phaseTwoDuration = rocketMimoFactory.phaseTwoDuration();
    ...
    function userMaxAllocation(address account) public view returns (uint256) {
        ...
        uint256 count = IERC721(rocketMimoFactory.machineFiNFT()).balanceOf(account);
        if (count > 0) {
            allocation *= (5 * count);
        }
        if (pointShares > 0) {
    ...
    function withdrawETH(uint256 _amount) external isStopped(false) {
        ...
        if (feeAmount > 0) {
            _safeTransferETH(rocketMimoFactory.penaltyCollector(), feeAmount);
        }
    ...
    function allowEmergencyWithdraw() external {
        require(
            msg.sender == Ownable(address(rocketMimoFactory)).owner(),
            "caller is not rocketMimoFactory owner"
        );
        stopped = true;
        emit Stopped();
    }
    ...
    function skim() external {
        ...
        address penaltyCollector = rocketMimoFactory.penaltyCollector();
        ...
        if (excessEth > 0) {
            _safeTransferETH(penaltyCollector, excessEth);
        }
    }
    ...
    function setMasterChefPoint(address _masterChefPoint, uint256 _shares) external {
        require(
            msg.sender == Ownable(address(rocketMimoFactory)).owner(),
            "caller is not rocketMimoFactory owner"
        );
